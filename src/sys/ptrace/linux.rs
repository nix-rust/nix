...
/// Reads a word from a process user area at the given address
pub fn read_user(pid: Pid, addr: AddressType) -> Result<c_long> {
    ptrace_peek(Request::PTRACE_PEEKUSER, pid, addr, ptr::null_mut())
}
...
/// Writes a word into the processes user area at the given address
///
/// # Safety
///
/// The `data` argument is passed directly to `ptrace(2)`.  Read that man page
/// for guidance.
pub unsafe fn write_user(pid: Pid, addr: AddressType, data: *mut c_void) -> Result<()> {
    ptrace_other(Request::PTRACE_POKEUSER, pid, addr, data).map(drop)
}
