...
impl<'a> AioCb<'a> {
    ...
    /// }
    /// assert_eq!(aiocb.aio_return().unwrap() as usize, LEN);
    /// let mut buffer = aiocb.into_boxed_mut_slice().unwrap();
    ...
    ///
    /// The returned object will the the argument that was passed to
    /// `from_boxed_mut_slice` when this `AioCb` was constructed.
    pub fn into_boxed_slice(mut self) -> Option<Box<Borrow<[u8]>>> {
        self.boxed_slice()
    }

    /// Consumes the `aiocb` and returns its inner boxed mut slice, if any.
    ///
    /// The returned object will be the argument that was passed to
    /// `from_boxed_mut_slice` when this `AioCb` was constructed.  This method
    /// is especially useful when reading into a type that does not support
    /// shared ownership.
    pub fn into_boxed_mut_slice(mut self) -> Option<Box<BorrowMut<[u8]>>> {
        self.boxed_mut_slice()
    }
