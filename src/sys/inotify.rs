...
use std::ffi::CString;
use errno::Errno;
use nix::{NixError, NixResult};
use fcntl::{Fd, O_CLOEXEC, O_NONBLOCK};
...
#[inline]
pub fn inotify_add_watch(fd: Fd, path: &str, mask: EventFlags) -> NixResult<Fd> {
    let res = unsafe {
...
#[inline]
pub fn inotify_rm_watch(fd: Fd, wd: u32) -> NixResult<()> {
